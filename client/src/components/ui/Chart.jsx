...
const ChartTooltipContent = forwardRef(({
  ...
  const tooltipLabel = useMemo(() => {
    ...
    return value ? <div className={`font-medium ${labelClassName}`}>{value}</div> : null;
  }, [label, labelFormatter, payload, hideLabel, labelClassName, config, labelKey]);

  if (!active || !payload?.length) return null;

  return (
    <div ref={ref} className={`grid min-w-[8rem] gap-1.5 rounded-lg border bg-background px-2.5 py-1.5 text-xs shadow-xl ${className}`}>
      {tooltipLabel}
      <div className="grid gap-1.5">
        {payload.map((item, index) => (
          <div key={item.dataKey} className="flex w-full gap-2 items-center">
            {!hideIndicator && <div className="h-2.5 w-2.5 rounded bg-[--color-bg]" style={{ backgroundColor: color || item.payload.fill || item.color }} />}
